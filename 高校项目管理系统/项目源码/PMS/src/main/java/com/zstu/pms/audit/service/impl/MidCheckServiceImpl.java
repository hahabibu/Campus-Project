...
public class MidCheckServiceImpl implements MidCheckService{
	...
	public List<DeclareRecordDTO> listDeclareRecord(DeclareRecord declareRecord) {
		...
		if(declareRecord!=null) {
			...
			// 初始化只显示已立项的记录
			criteria.andProcessStatusEqualTo(ConstantUtils.PROCESS_STATUS_SET);
		}
	...
	public boolean PassMidCheck(String declareId, String result) {
    	...
    	// 设置处理状态
    	findDeclareRecord.setProcessStatus(ConstantUtils.PROCESS_STATUS_MIDCHECK_PASS);
	...
	public boolean unPassMidCheck(String declareId, String result) {
		DeclareRecord findDeclareRecord = this.getDeclareRecord(declareId);
    	// 设置处理状态
    	findDeclareRecord.setProcessStatus(ConstantUtils.PROCESS_STATUS_MIDCHECK_UNPASS);
    	findDeclareRecord.setResult(result);
    	int i = declareRecordMapper.updateByPrimaryKey(findDeclareRecord);
		return i>0;
	}
