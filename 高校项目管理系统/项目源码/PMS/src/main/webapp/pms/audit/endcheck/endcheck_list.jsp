...
<body>
	...
	<form class="form-inline" method="post"
		action="${pageContext.request.contextPath }/audit/endcheck/listDeclareRecord.action">
	...
	<div align="center">
		<div class="alert alert-warning"
			...
			<p align="center" style="color: red;">结题验收管理-${info}</p>
		</div>
	</div>
			...
			<table class="table  table-condensed table-striped">
				...
				<c:forEach var="declareRecord" items="${list }">
					<tr>
						...
						<c:if test="${declareRecord.processStatus=='2' }">
							<td>
	...
	<script type="text/javascript">
		function passCheck(declareId) {
			var info = window.confirm("确认通过该申报记录的结题审核?");
			if (info) {
				var result = prompt("请填写审核结果", "通过结题审核");
				if (result != null) {
					if(result != ""){
						alert("审核结果已提交："+result);
						var url = "${pageContext.request.contextPath }/audit/endcheck/passMidCheck.action?declareId="
		...
		function unPassCheck(declareId) {
			var info = window.confirm("确认驳回该申报记录的结题审核?");
			if (info) {
				var result = prompt("请填写审核结果", "驳回结题审核");
				if (result != null) {
					if(result != ""){
						alert("审核结果已提交："+result);
						var url = "${pageContext.request.contextPath }/audit/endcheck/unPassMidCheck.action?declareId="
								+ declareId + "&result=" + result;
						window.location.href = url;
					} else {
						alert('审核结果输入不能为空！');
