...
<body>
	...
	<form class="form-inline" method="post"
		action="${pageContext.request.contextPath }/audit/approval/listDeclareRecord.action">
	...
	<div align="center">
		<div class="alert alert-warning"
			...
			<p align="center" style="color: red;">立项管理管理-${info}</p>
		</div>
	</div>
			...
			<table class="table  table-condensed table-striped">
				...
				<c:forEach var="declareRecord" items="${list }">
					<tr>
						...
						<td>
							<%-- <a href="${pageContext.request.contextPath }/audit/endcheck/showDeclareRecord.action?declareId='${declareRecord.declareId }'">查看</a> --%>
							<a href="javascript:showData('${declareRecord.declareId }')">查看</a>
						</td>
						<c:if test="${declareRecord.processStatus=='0' }">
							<td>
								<%-- <input type="submit" class="btn btn-info" onclick="passCheck('${declareRecord.declareId }')" value="通过审核" /> --%>
								<a href="javascript:passCheck('${declareRecord.declareId }')">批准立项</a>
							</td>
							<td>
								<%-- <input type="submit" class="btn btn-default" onclick="unPassCheck('${declareRecord.declareId }')" value="驳回记录" /> --%>
								<a href="javascript:unPassCheck('${declareRecord.declareId }')">驳回立项</a>
							</td>
						</c:if>
					</tr>
				</c:forEach>
	...
	<script type="text/javascript">
		function passCheck(declareId) {
			var info = window.confirm("确认通过立项?");
			if (info) {
				var result = prompt("请填写审核结果", "通过立项");
				if (result != null) {
					if(result != ""){
						alert("审核结果已提交："+result);
						var url = "${pageContext.request.contextPath }/audit/approval/passApproval.action?declareId="
		...
		function unPassCheck(declareId) {
			var info = window.confirm("确认驳回立项?");
			if (info) {
				var result = prompt("请填写审核结果", "驳回立项");
				if (result != null) {
					if(result != ""){
						alert("审核结果已提交："+result);
						var url = "${pageContext.request.contextPath }/audit/approval/unPassApproval.action?declareId="
								+ declareId + "&result=" + result;
						window.location.href = url;
					} else {
						alert('审核结果输入不能为空！');
